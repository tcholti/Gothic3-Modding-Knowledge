...
| discoverable project continuation / interrupted-recovery first hop | root `README.md` | routes to `docs/SESSION_ENTRYPOINT.md` + adopted CAM Recovery Lock | canonical project first hop changes or no longer routes ordinary/recovery startup correctly | current objective changes inside the maintained session entry point |
| current objective / latest durable boundary / immediate next responsibility | `docs/SESSION_ENTRYPOINT.md` | root `README.md` discoverable route + Git durable state | active responsibility or prerequisite/blocker changes what a fresh context should do next | minor substep that does not change continuation |
